...
const handlers: Record<string, Handler> = {
  ...
  alnum: {
    ...
    parse({ stack, exprs }, token, next) {
      if (next && (rank(next.kind) === 0 || handlers[next.kind].left)) {
        stack.push(token)
      } else {
        exprs.push({ token })
      }
...
function reduce(
  ...
  while (stack.length > 0) {
    ...
    // TODO should infix operators be left or right associative?
    ...
    const rhs = exprs.pop()
    if (!rhs) {
      return { code: 'invalid_expr', pos: token.pos }
    }
    exprs.push(
      token.kind !== 'alnum'
        ? { token, lhs: exprs.pop(), rhs }
        : { token, rhs },
    )
  }
}
...
export default function (source: string): {
  ...
  const error = reduce(parser, 'dedent')
  if (error) {
    return { error }
  }

  // TODO a non-empty stack should also be an error
  if (exprs.length !== 1) {
